...
-dit : runs the container having an interactive terminal which is detached, i.e. we don't get a bash shell after the conatiner is launched
...
If everything is working fine in the development container, then the QTA guy will trigger the Job 3 using remote URL we will provide to him (the remote URL will be provided later in the job 3)
 ...
 Job 2 : Jenkins job which will pull from dev branch and deploy the code in development testing httpd container, an email will be sent from here to QAT notifying a push is made, for testing
Finally Job 3
 Job 3 : This job will be triggered by QAT ( quality assurance team ) by a remote URL that you will provide to them , this job will merge dev branch into master branch and call Job 1 again
...
Set it to build
A URL will be shown there as follows : JENKINS_URL/job/MergeDevInMaster/build?token=TOKEN_NAME or /buildWithParameters?token=TOKEN_NAME
...
One last things, if you observed closely we are using sudo before some commands, so what is that
